...
export const Route = createFileRoute("/_authenticated/app/audits/$id")({
  head: () => ({
    meta: [
                      ...
                      {audit.auditFindings.map((finding) => (
                        <li key={finding.id} className="flex items-start gap-3 rounded-lg border border-border/60 bg-background/40 px-4 py-3">
                          <div className="min-w-0 flex-1">
                            ...
                            {finding.description && (
                              <p className="mt-0.5 text-xs text-muted-foreground line-clamp-2">{finding.description}</p>
                            )}
                            {finding.dueDate && (
                          ...
                          <div className="flex flex-col items-end gap-1.5 shrink-0">
                            ...
                            <span className={cn("rounded-full border px-2 py-0.5 text-[10px] font-medium", finding.status === "CLOSED" ? "bg-green-500/10 text-green-600 border-green-500/20" : "bg-yellow-500/10 text-yellow-600 border-yellow-500/20")}>
                              {finding.status}
                            </span>
                          </div>
                        </li>
                  ...
                  <div className="mt-4 space-y-3">
                    {[
                      ...
                      <div key={step.label} className="flex items-center gap-3">
                        <div className={cn("flex h-7 w-7 items-center justify-center rounded-full border", step.done ? "border-green-500/30 bg-green-500/10 text-green-600" : "border-border/60 bg-muted/40 text-muted-foreground/50")}>
