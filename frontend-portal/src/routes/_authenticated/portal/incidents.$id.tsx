...
import {
  ArrowLeft, ShieldAlert, MapPin, User, Calendar, Tag,
  Building2, ClipboardList, AlertCircle,
} from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/portal/incidents/$id")({
  head: () => ({
    meta: [
              ...
              <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="shrink-0">
                  <span className={cn("text-xs font-medium px-2.5 py-1 rounded-lg border",
                    incident.priority === "CRITICAL" ? "bg-red-500/10 text-red-600 border-red-500/20"
                      : incident.priority === "HIGH" ? "bg-yellow-500/10 text-yellow-600 border-yellow-500/20"
              ...
              <div className="space-y-6 lg:col-span-2">
                {incident.description && (
                  <SurfaceCard className="p-6">
                    ...
                    <p className="mt-3 whitespace-pre-line text-sm leading-relaxed text-foreground/85">{incident.description}</p>
                  </SurfaceCard>
                )}
                      ...
                      {incident.capas.map((capa) => (
                        <li key={capa.id} className="flex items-start gap-3 rounded-lg border border-border/60 bg-background/40 px-4 py-3">
                          <div className="min-w-0 flex-1">
                            ...
                            {capa.dueDate && <p className="mt-0.5 text-xs text-muted-foreground">Due {new Date(capa.dueDate).toLocaleDateString()}</p>}
                          </div>
                          <span className={cn("shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-medium",
                            capa.status === "COMPLETED" ? "bg-green-500/10 text-green-600 border-green-500/20" : "bg-yellow-500/10 text-yellow-600 border-yellow-500/20"
                          )}>
                            {capa.status}
                          </span>
                        </li>
