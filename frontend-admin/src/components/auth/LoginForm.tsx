  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!["super_admin", "tenant_admin"].includes(role)) {
        ...
        setErrors({ form: "Access denied. This portal is for administrators only." });
        return;
      }
      brandingStore.invalidate();
      brandingStore.fetch();
      navigate({ to: (search.redirect?.startsWith("/admin") ? search.redirect : "/admin/dashboard") as any });
    } catch (e: unknown) {
      const msg =
